var MyWin = new Win();
var imgfile = "../Images/background/"; //设置图片路径，相对或绝对都行
...
function Win() {
    this.Create = function(mask, title, wbody, w, h, l, t) {
        ...
        if (mask != "no") {
            ...
            ndiv.style.cssText = "width:" + sw + "px;height:" + sh + "px;left:0px;top:0px;position:absolute;overflow:hidden;background:#fff;filter:alpha(opacity=50); opacity:0.5;-moz-opacity:0.5;";
            ...
            if (Ie) {
                ...
                niframe.style.filter = "alpha(opacity=80)";
        ...
        for (var i = 0; i < 9; i++) {
            ...
            if (wintag[i][3]) {
                ...
                temp.style.cssText = "float:left;filter:alpha(Opacity=90,style=0);opacity:0.90;height:" + wintag[i][1] + "px;width:" + wintag[i][2] + "px;background:#f7f7f7;border:1px solid #E2D2D3;overflow:hidden;padding:0px";
            }
        }
        mytie.childNodes[1].innerHTML = "<div style=\"position:absolute;overflow:hidden;height:15px;top:12px;padding-left:4px;padding-right:4px;\"></div><div style=\"position:absolute;background:url('" + imgfile + "clo.gif');overflow:hidden;width:48px;height:20px;top:8px !important;right:16px\" title=\"关闭窗口\" onclick=\"MyWin.Close('win" + Winid + "',10); MyWin.ndiv('ndiv" + Winid + "',10);\"></div>";
    ...
    this.Body = function(Id, wbody) {
        ...
        if (wbody.slice(0, 4) == "[pg]")
            o.childNodes[1].childNodes[1].innerHTML = "<iframe onfocus=\"MyWin.Show('" + Id + "',this)\" src='" + wbody.slice(4) + "' frameBorder='0' marginHeight='0' marginWidth='0' scrolling='auto' width='100%' height='100%'></iframe>";
        else
